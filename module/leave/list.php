				...
				<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
				  ...
				  <tbody>
					...
					$cur = $mydb->loadResultList();
				  		...
				  		global $mydb;
			  		$mydb->setQuery("select  * from tblleave where LEAVESTATUS='PENDING' AND `EMPLOYID` IN ( SELECT `EMPLOYID` from tblemployee WHERE `COLLEGE`='". $_SESSION['COLLEGE']  ."' AND `DEPARTMENT`='". $_SESSION['DEPARTMENT']  ."')  " );
					$cur = $mydb->loadResultList();
				  		foreach ($cur as $Defaults) {
					  		echo '<tr>';
				  		...
				  		global $mydb;
			  		$mydb->setQuery("select  * from tblleave where LEAVESTATUS='PENDING' AND `EMPLOYID` IN ( SELECT `EMPLOYID` from tblemployee WHERE `COLLEGE`='". $_SESSION['COLLEGE']  ."' )  " );
					$cur = $mydb->loadResultList();
				  		foreach ($cur as $Defaults) {
					  		echo '<tr>';
